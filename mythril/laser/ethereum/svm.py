...
class LaserEVM:
    ...
    def _execute_transactions_incremental(self, address, txs=None):
        ...
        for i in range(self.transaction_count):
            ...
            if self.use_reachability_check: # v
                if isinstance(self.strategy, DelayConstraintStrategy): # f
                    open_states = []
                    for state in self.open_states:
    ...
    def exec(self, create=False, track_gas=False) -> Optional[List[GlobalState]]:
        ...
        for global_state in self.strategy:
            ...
            if create and self._check_create_termination(): # f
                log.debug("Hit create timeout, returning.")
                return final_states + [global_state] if track_gas else None

            if not create and self._check_execution_termination(): # f
            ...
            if self.strategy.run_check() and ( # v & f
